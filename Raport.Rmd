...
Podobnie jak w przypadku długości edukacji, będziemy analizować dane dotyczące PKB wyłącznie z 2015 roku. Wykres prezentuje się następująco
...
```
W przeciwieństwie do poprzedniego przypadku, dane te nie mają zależności liniowej, a kształtem przypomina bardziej logarytmiczną. W tym celu, zrobimy transformację danych do postaci liniowej, żeby móc później skorzystać z metody najmniejszych kwadratów. Innymi słowy $\tilde{x} = \log(x)$
...
```
Następnie dopasowujemy prostą regresji z MNK:

```{r reg_gdp, fig.cap="\\label{fig:reg_gdp} Prosta regresji dla przetransformowanych danych", fig.align="center", fig.width = 3, fig.height = 2}
reg2 <- regression(log(df2$GDP), df2$life_expectancy)
reg2
```
Ostatecznie wykonujemy transformację odwrotną dla prostej regresji co w rezultacie da krzywą logarytmiczną dopasowaną do danych.

```{r reg_gdp_final, fig.cap="\\label{fig:reg_gdp_final} Regresja logarytmiczna orginalnych danych", fig.align="center", fig.width = 3, fig.height = 2}
c(a, b) %<-% regression(log(df2$GDP), df2$life_expectancy, FALSE)
xs <- seq(0.1, max(df2$GDP), 10)
reg2_final <- ggplot() + geom_point(aes(df2$GDP, df2$life_expectancy), alpha = 0.5) + geom_line(aes(xs, a*log(xs) + b), col='red', linewidth=1)
reg2_final
```
